...
def run_cli_tuning(arch_name, n_trials=50, use_subsample=False, subsample_ratio=0.2):
    print(f"==================================================")
    print(f"[START] memulai CLI Optuna Tuning untuk Model: {arch_name.upper()}")
    print(f"==================================================")
    ...
    if arch_name in ['gru', 'lstm', 'rnn']:
        ...
        cfg['tuning']['search_space'] = {
            # Decoder-only: NO patch_len / stride (tidak pakai patching)
            'd_model': [32, 64, 128],       # Mulai 32 karena Head = lookback×d_model (bisa meledak)
            'n_layers': [2, 6],             # Decoder-only butuh lebih dalam dari encoder-decoder
            'n_heads': [4, 8],              # Dibatasi agar d_model/n_heads >= 8
            'ff_dim': [64, 512],            # 2x-4x d_model
            'dropout': [0.05, 0.35],        # Lebih rendah, causal mask sudah regularisasi alami
            'learning_rate': [5e-5, 5e-3],  # Lebih lebar, decoder-only konvergen lambat
            'batch_size': [128, 128],       # DIKUNCI di 128
            'lookback': [72, 336, 24]       # 3-14 hari, hati-hati lookback besar = params besar
        }
    else:
    ...
    print(f"\n[DONE] Tuning Selesai! Riwayat Detail (Excel) disimpan di: {csv_file}")
