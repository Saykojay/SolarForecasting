...
| **`batch_size`** <br>*(Umpan Per Update)* | **`32`** | *Fixed* | Dikunci pada 32. Meskipun GPU modern biasanya menyukai batch besar untuk utilisasi maksimal, pada laptop yang rentan panas/VRAM minim, mematok di 32 memastikan konsumsi daya dan *memory-footprint* stabil terkontrol selama proses berjam-jam. |
...
| **`use_bidirectional`** <br>*(Evaluasi Bolak-Balik)* | **`False`** | *Boolean* | Layer `Bidirectional` memakan waktu **DUA KALI LIPAT** lebih lambat (kiri-kanan, kanan-kiri). Fitur ini telah "**Dimatikan Mutlak**" (selalu *False*) demi memangkas waktu komputasi (GPU Hour) sebanyak 50% di laptop kesayangan. |
