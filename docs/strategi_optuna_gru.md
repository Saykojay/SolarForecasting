...
| **`batch_size`** <br>*(Umpan Per Update)* | **`32` - `128`** | *Categorical* | GRU adalah model sekuensial langkah-demi-langkah (Time-Step by Time-Step) di dalam loop perhitungan GPU-nya. Jika memakai rentang Transformers (256 - 512), matriks perhitungannya akan melelehkan VRAM karena loop iterasi dalam Keras harus dikomputasi untuk setiap langkah waktu serentak. Cukup batasi maksimal 128 untuk *stability loss function*. |
